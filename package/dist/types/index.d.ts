export type SchedulerJobStatus = 'pending' | 'queued' | 'running' | 'completed' | 'failed_retryable' | 'failed' | 'dead_letter' | 'cancelled' | 'expired';
...
export type JobHandler<TPayload extends JsonObject = JsonObject, TResult = unknown> = (payload: TPayload, context: JobContext) => Promise<TResult> | TResult;
...
export interface SchedulerQueue {
    enqueue(record: SchedulerJobRecord, options: JobOptions): Promise<{
        jobId: string;
        existing: boolean;
    }>;
